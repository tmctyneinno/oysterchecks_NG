...
use App\Jobs\ResendAddressWebhookJob;
...
class UpdateVerificationStatus extends Controller
{
    ...
    public function resendWebhook()
    {
        ...
        AddressVerificationDetail::select('id', 'reference_id')->chunkById(100, function ($addresses) use ($token) {
            foreach ($addresses as $address) {
                ResendAddressWebhookJob::dispatch($address->reference_id, $token);
            }
        });

        return back()->with('message', 'Webhook resend has been queued for all address verifications.');
    }
}
